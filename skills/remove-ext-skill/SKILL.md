...
사용자가 `/blog-poster:remove-ext-skill`로 호출하여 기존 외부호출 스킬(ext-{대상플러그인})을 제거할 수 있게 함.
...
사용자가 `/blog-poster:remove-ext-skill` 호출 시.
...
- `skills/help/SKILL.md`의 명령 테이블에서 `/blog-poster:ext-{대상플러그인}` 행 제거
