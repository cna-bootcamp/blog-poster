...
사용자가 `/blog-poster:add-ext-skill`로 호출하여
...
사용자가 `/blog-poster:add-ext-skill` 호출 시.
...
- `.dmap/blog-poster/requirements.md` (요구사항 정의서)
...
- `skills/help/SKILL.md`의 명령 테이블에 `/blog-poster:ext-{대상플러그인}` 추가
...
Use the Skill tool to invoke the `blog-poster:ext-{대상플러그인}` skill with all arguments passed through.
```
